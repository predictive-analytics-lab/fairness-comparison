...
algos_to_plot = c(
  "FairGPopp*TPR0.6", "FairGPopp*TPR0.7", "FairGPopp*TPR0.8","FairGPopp*TPR0.9", 
  "FairGPoppTPR0.6", "FairGPoppTPR0.7", "FairGPoppTPR0.8","FairGPoppTPR0.9"
  )
...
make_scatter_figure = function(name, var1="race-TPRDiff", var2="accuracy", algos=algos_to_plot, display="off", statistic = "off") {
  ...
  df = read.csv(str_c(name, "_numerical-binsensitive.csv"), check.names=FALSE) %>%
    ...
    mutate(algorithm=recode(algorithm, "FairGPopp*TPR0.6"="FairGPopp*, TPR=0.6"))%>%
    mutate(algorithm=recode(algorithm, "FairGPopp*TPR0.7"="FairGPopp*, TPR=0.7"))%>%
    mutate(algorithm=recode(algorithm, "FairGPopp*TPR0.8"="FairGPopp*, TPR=0.8"))%>%
    mutate(algorithm=recode(algorithm, "FairGPopp*TPR0.9"="FairGPopp*, TPR=0.9"))%>%
    
    mutate(algorithm=recode(algorithm, "FairGPoppTPR0.6"="FairGPopp, TPR=0.6"))%>%
    mutate(algorithm=recode(algorithm, "FairGPoppTPR0.7"="FairGPopp, TPR=0.7"))%>%
    mutate(algorithm=recode(algorithm, "FairGPoppTPR0.8"="FairGPopp, TPR=0.8"))%>%
    mutate(algorithm=recode(algorithm, "FairGPoppTPR0.9"="FairGPopp, TPR=0.9"))
  ...
  switch(display,
         ...
         all = ggplot(dataFrame, aes_q(x=x_var, y=y_var, colour=quote(algorithm))) + geom_point(size=3, aes(shape = algorithm)) +
           scale_shape_manual(values=symbol) +
...
for (i in 1:N_file) {
  ...
  # q_1 = make_scatter_figure(file_name[i], var1=var1_name, var2=target_name, algos=plot_for_in, display = "all")
  # print(q_1)
  # export_name = paste(file_name[i], "_odds_in.eps", sep="")
  # ggsave(export_name, q_1)
  
  q_2 = make_scatter_figure(file_name[i], var1=var1_name, var2=target_name, display = "all")
  print(q_2)
